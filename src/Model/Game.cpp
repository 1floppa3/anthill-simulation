...
{
	food.emplace_back(Utils::Random::random(1500), Utils::Random::random(1000), 1);
}

Ant* Game::find_closest_ant(const sf::Vector2f& position, float field_of_vision) const
{
	if (anthill->ants.size() == 0)
		return nullptr;

	Ant* closest_ant = nullptr;
